...
def quantize_with_nncf_calibrated(model_path: Path, output_path: Path, n_samples: int = 100):
    """
    ...
    calibration_dataset = nncf.Dataset(calibration_data_gen())
...
def main():
    ...
    parser.add_argument("--model", type=str, required=True, help="Model name (e.g., large-v3-turbo)")
    parser.add_argument("--method", type=str, default="weights", 
                        choices=["weights", "calibrated"],
                        help="Quantization method: 'weights' (fast, no data) or 'calibrated' (slow, better)")
    parser.add_argument("--samples", type=int, default=100, help="Calibration samples (for calibrated method)")
